...
class CurrentLocationViewController: UIViewController, CLLocationManagerDelegate, CAAnimationDelegate {
    ...
    func updateLabels() {
        if let location = location {
            ...
            else { statusMessage = ""; showLogoView() }
    ...
    func configureGetButton() {
        ...
        if updatingLocation {
            ...
            if let spinner = view.viewWithTag(spinnerTag) { spinner.removeFromSuperview() }
        }
    }
    ...
    func loadSoundEffect(_ name: String) {
        if let path = Bundle.main.path(forResource: name, ofType: nil) {
            ...
            if error != kAudioServicesNoError { print("Error code \(error) loading sound: \(path)") }
        }
    }
    ...
    func playSoundEffect() { AudioServicesPlaySystemSound(soundID) }
